  ...
  module Utilities
    ...
    module Encoding
      def self.encode_incoming(string, encoding)
        ...
        if encoding == :irc
          ...
          if !string.valid_encoding?
            string.force_encoding("CP1252").encode!("UTF-8", options: {invalid: :replace, undef: :replace})
          end
        else
          string.force_encoding(encoding).encode!(options: {invalid: :replace, undef: :replace})
      ...
      def self.encode_outgoing(string, encoding)
        ...
        return string.encode!(encoding, options: {invalid: :replace, undef: :replace}).force_encoding("ASCII-8BIT")
      end
    end
  end
end
